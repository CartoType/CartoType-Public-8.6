...
CartoType::Result CCartoTypeDemoApp::InitializeCartoType()
    ...
    if (!error)
        error = LoadFont(app_path,"DejaVuSans-Oblique.ttf");
    if (!error)
        error = LoadFont(app_path,"DejaVuSans-BoldOblique.ttf");
    if (!error)
        error = LoadFont(app_path,"DejaVuSerif.ttf");
    if (!error)
        error = LoadFont(app_path,"DejaVuSerif-Bold.ttf");
    if (!error)
        error = LoadFont(app_path,"DejaVuSerif-Italic.ttf");
    if (!error)
        error = LoadFont(app_path,"DejaVuSerif-BoldItalic.ttf");
    if (!error)
